...
const Header = () => {
   ...
   const hideButtons = (location.pathname === "/login" || location.pathname === "/register") && isMobile;
      ...
      <header className='flex'>
         ...
         {!hideButtons && (
            <div>
               ...
               {user 
                  ? <button className='littleButton buttonYellow' onClick={logout}>Log Out</button>
                  : <Link to="/login"><button className='littleButton'>Login</button></Link>}
            </div>
         )}
      </header>
   );
